                ...
                <div class="card">
                    <div class="card-body">
                        <form
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6 col-12">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <br>
                                        @if ($data->get_thumb)
                                            <div class="img-box position-relative d-inline-block me-2 mb-2"
                                                id="media-{{ $data->get_thumb->id }}">
                                                <img src="{{ asset($data->get_thumb->file_url) }}" height="70"
                                                ...
                                                <button type="button" class="delete-media-btn"
                                                    data-id="{{ $data->get_thumb->id }}" data-type="image"
                                ...
                                <div class="form-group col-md-6 col-12">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <br>
                                        {{-- @dd($data->get_gallery_images()) --}}

                                        @if ($data->get_gallery()->count() > 0)
                                            @foreach ($data->get_gallery() as $img)
                                                <div class="img-box position-relative d-inline-block mr-2 mb-2"
                                                    id="media-{{ $img->id }}">
                                                    <img src="{{ asset($img->file_url) }}" height="70"
                                                        class="rounded border shadow-sm">
                                                    <div class="loading-overlay">
                                                        <div class="custom-spinner"></div>
                                                    </div>
                                                    <button type="button" class="delete-media-btn"
                                                        data-id="{{ $img->id }}" data-type="gallery"
                                                        data-product="{{ $data->id }}">×
                                                    </button>
                                                </div>
                                            @endforeach
                                        ...
                                        <small>*image size should be square</small>
                                    </div>
                                </div>
                                        ...
                                        <div class="col-md-10 col-12" id="attribute-table-put">
                                            @if (count($data->get_variants) > 0)
                                                <table class="table table-bordered my-2">
                                                    ...
                                                    <tbody>
                                                        @foreach ($data->get_variants as $key => $attribute)
                                                            <tr>
                                                                @php
                                                                    $variantName = strtolower($attribute->variant);
                                                                    $variantSku = strtolower($attribute->sku);
                                                                @endphp
                                                                {{-- @dd($attribute->get_variant_options) --}}
                                                                <td>
                                                                    <input type="hidden" name="attribute_id[]"
                                                                        value="{{ $attribute->id }}">
                                                                    <input type="text" name="variant_name[]"
                                                                        value="{{ $variantName }}" class="form-control"
                                                                        readonly>

                                                                    {{-- Existing variant options --}}
                                                                    @foreach ($attribute->get_variant_options as $option)
                                                                        <input type="hidden"
                                                                            name="combination_id[{{ $key }}][]"
                                                                            value="{{ strtolower($option->choice_attribute_item_name) }}">
                                                                    @endforeach
                                                                </td>
                                                                ...
                                                                <td>
                                                                    <input type="text" name="variant_sku[]"
                                                                        value="{{ $variantSku }}" class="form-control"
                                                                        readonly>
                                                                </td>
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6 col-12">
                                    <label for="video_url_e">Video URL</label>
                                    <input type="text" class="form-control" id="video_url_e" name="video_url"
...
@section('js')
    ...
    <script>
        $(document).on('click', '.delete-media-btn', function() {
            let btn = $(this);
            let mediaId = btn.data('id');
            let type = btn.data('type');
            let productId = btn.data('product');
            let box = btn.closest('.img-box');
            let overlay = box.find('.loading-overlay');
            ...
            $.ajax({
                url: "{{ route('product.media.delete') }}",
                method: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    media_id: mediaId,
